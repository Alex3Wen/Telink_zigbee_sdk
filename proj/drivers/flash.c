...
_attribute_flash_code_ static inline int flash_is_busy(){
    return mspi_read() & 0x01;                //  the busy bit, pls check flash spec
}

_attribute_flash_code_ static void flash_send_cmd(u8 cmd){
    mspi_high();
    sleep_us(1);
    mspi_low();
    mspi_write(cmd);
...
_attribute_flash_code_ static void flash_send_addr(u32 addr){
...
_attribute_flash_code_ static void flash_wait_done()
{
    sleep_us(100); 
...
_attribute_flash_code_ void flash_erase_sector(u32 addr){
    u8 r = irq_disable();
...
_attribute_flash_code_ void flash_write_page(u32 addr, u32 len, u8 *buf){
    u8 r = irq_disable();
...
_attribute_flash_code_ void flash_read_page(u32 addr, u32 len, u8 *buf){
    u8 r = irq_disable();
...
#if 1            //  没有用到的都占用 ram,  所以只有使用的时候才打开
...
_attribute_ram_code_ void flash_power_down(){
    u8 r = irq_disable();
    flash_send_cmd(FLASH_POWER_DOWN);
    irq_restore(r);
}

_attribute_ram_code_ void flash_release_pwrdn(){
    u8 r = irq_disable();
    flash_send_cmd(FLASH_RELEASE_PWRDN);
    irq_restore(r);
}
