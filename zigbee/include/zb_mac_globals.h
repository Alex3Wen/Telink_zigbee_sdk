...
#define ZB_MAC_SET_DEVICE_TYPE(device_type) (ZCTRL->mac_ctx.dev_type = (device_type))

#if 0
#define ZB_SET_MAC_SYNC_IO() (ZCTRL->mac_ctx.mac_io_sync_mode = 1)
#define ZB_SET_MAC_ASYNC_IO() (ZCTRL->mac_ctx.mac_io_sync_mode = 0)
#define ZB_CHECK_MAC_SYNC_IO() (ZCTRL->mac_ctx.mac_io_sync_mode)
#else
...
#define ZB_SET_MAC_STATUS(status) (ZCTRL->mac_ctx.mac_status = (status))
#define ZB_GET_MAC_STATUS() (zb_mac_status_t)((zb_uint8_t)ZCTRL->mac_ctx.mac_status + 0)
...
#define ZB_MAC_ACK_WITH_PENDING   0x80
...
#define ZB_MAC_SET_ACK_WITH_PENDING()   (MAC_CTX().mac_flags2 |= ZB_MAC_ACK_WITH_PENDING)
#define ZB_MAC_GET_ACK_WITH_PENDING()   (MAC_CTX().mac_flags2 & ZB_MAC_ACK_WITH_PENDING)
#define ZB_MAC_CLEAR_ACK_WITH_PENDING() (MAC_CTX().mac_flags2 &= ~ZB_MAC_ACK_WITH_PENDING)
...
typedef enum zb_mlme_state_e
{
	ZB_MLME_STATE_IDLE,
	ZB_MLME_STATE_ASS_REQ,
	ZB_MLME_STATE_ASS_RESP,
} ZB_PACKED_STRUCT
zb_mlme_state_t;
...
{
  zb_uint8_t            silent_start;
  zb_mlme_state_t       state;
  ...
  volatile zb_uint8_t   tx_cnt; /* tx counter, just to save tx number */
