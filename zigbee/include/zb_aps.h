...
typedef struct{
	zb_uint8_t fEn;
	zb_uint8_t APSDE_DATAREQ_DST_ADDR_MODE;
	zb_uint16_t DST_NWK_ADDR;
    zb_uint8_t destEndpoint;
    zb_uint8_t srcEndpoint;
}apsde_dataCnf_detail_t;
...
typedef enum zb_aps_status_e
{
  ...
  ZB_APS_STATUS_DUPLICATE_ENTRY       = 0xb8
} zb_aps_status_t;
...
@endcode
 */
#define ZB_APS_HDR_CUT_P(packet, ptr, type)                   \
  ZB_BUF_CUT_LEFT(packet, zb_aps_full_hdr_size(ZB_BUF_BEGIN(packet)), ptr, type)
...
*/
typedef struct zb_apsme_remove_group_req_s
{
  zb_uint16_t group_address;    /*!< The 16-bit address of the group being added.  */
  zb_uint8_t  endpoint;         /*!< The endpoint to which the given group is being added.  */
...
*/
typedef struct zb_apsme_remove_group_conf_s
{
  zb_uint8_t  status;
  zb_uint16_t group_address;    /*!< The 16-bit address of the group being added.  */
  zb_uint8_t  endpoint;         /*!< The endpoint to which the given group is being added.  */
...
/**
   APSME-REMOVE-ALL-GROUP.request primitive parameters
*/
typedef struct zb_apsme_remove_all_group_req_s
{
  zb_uint8_t  endpoint;         /*!< The endpoint to which the given group is being added.  */
} zb_apsme_remove_all_group_req_t;


/**
   APSME-REMOVE-ALL-GROUP.confirm primitive parameters
*/
typedef struct zb_apsme_remove_all_group_conf_s
{
  zb_uint8_t  status;
  zb_uint8_t  endpoint;         /*!< The endpoint to which the given group is being added.  */
} zb_apsme_remove_all_group_conf_t;
...
*/
zb_uint8_t zb_apsme_add_group_request(zb_apsme_add_group_req_t *req) ;
zb_uint8_t zb_apsme_remove_group_request(zb_apsme_remove_group_req_t* req) ;
zb_uint8_t zb_apsme_remove_all_group_request(zb_apsme_remove_all_group_req_t* req);
void       aps_group_reset(void);
zb_aps_group_table_ent_t* aps_group_find(u16 groupAddr, u8 endpoint);
...
void zb_send_apsde_confirm(zb_uint8_t status);
