...
#include "../mac/transceiver/mac_trx_api.h"
 ...
 */
#ifndef ZB_CC25XX
#define ZB_MAC_GET_LQI(packet) *((zb_uint8_t*)ZB_BUF_BEGIN(packet) + ZB_BUF_LEN(packet) - 1)
#else
 ...
 */
#define ZB_MAC_CUT_HDR(packet, hlen, ptr, type)                         \
do                                                                \
{                                                                 \
  /* Save hdr offset */                                           \
  ...
  ZB_BUF_CUT_LEFT(packet, hlen, ptr, type);                             \
} while (0)
...
#define ZB_MAC_CUT_HDR_WITHOUT_TRAILER(_packet, _ptr, _type)               \
do                                                                \
{                                                                 \
  zb_ushort_t hlen;                                               \
  void *mac_hdr = ZB_BUF_BEGIN(_packet);                           \
  ...
  hlen = zb_mac_calculate_mhr_length(                             \
    ...
    ZB_FCF_GET_DST_ADDRESSING_MODE(mac_hdr),                      \
    ZB_FCF_GET_PANID_COMPRESSION_BIT(mac_hdr));                   \
  ZB_BUF_CUT_LEFT(_packet, hlen + MAC_SECUR_LEV5_KEYID1_AUX_HDR_SIZE * ZB_FCF_GET_SECURITY_BIT(mac_hdr) , _ptr, _type);                                        \
                                        \
                                                                  \
} while (0)
...
@endcode
*/
#define ZB_MAC_GET_BEACON_PAYLOAD(mhr, mhr_len, beacon_payload_p, type )        \
  beacon_payload_p = (type)(((zb_uint8_t *)mhr) + mhr_len + SUREPFRAME_SPEC_LENGTH \
...
#define ZB_MAC_GET_BEACON_ORDER( p_sfs ) ((zb_uint_t)((((zb_uint8_t*)(p_sfs))[ZB_PKT_16B_ZERO_BYTE]) & 0x0f))  //0x07))
 ...
 */
#define ZB_PIB_ACK_WAIT_DURATION()  (MAC_PIB().mac_ack_wait_duration)

/**
   Get mac max csma backoffs
 */
#define ZB_PIB_MAX_CSMA_BACKOFFS()  (MAC_PIB().mac_max_csma_backoffs)

/**
   Get mac min csma backoffs
 */
#define ZB_PIB_MIN_BE()  (MAC_PIB().mac_min_be)

/**
   Get mac min csma backoffs
 */
#define ZB_PIB_MAX_FRAME_RETRIES()  (MAC_PIB().mac_max_frame_retries)
...
#ifdef ZCP_TEST
extern zb_uint16_t RejectionAddrList[10];
extern zb_bool_t updateLQI;
#endif
...
/**
   ...
   @return Returns RET_OK on success and RET_ERROR on error.
 */
zb_ret_t zb_mlme_active_scan(zb_uint8_t param) ;
 ...
 */
zb_ret_t zb_mac_process_mlme_start(zb_uint8_t param) ;
...
/**
  ...
  @return RET_OK, RET_ERROR
*/
zb_ret_t zb_handle_associate_response(zb_uint8_t param);
...
*/
zb_ret_t zb_handle_data_request_cmd(zb_uint8_t param) ;
...
zb_ret_t zb_tx_coord_realignment_cmd(zb_bool_t is_broadcast, zb_ieee_addr_t orphaned_dev_ext_addr,
                                     zb_uint16_t orphaned_dev_short_addr, zb_uint8_t param);
...
*/
zb_ret_t zb_mac_send_comm_status(zb_buf_t *info_buf, zb_uint8_t mac_status, zb_buf_t *buffer) ;
...
#define ZB_WAIT_FOR_TX() while (0 == MAC_CTX().tx_cnt) ZB_GO_IDLE()
#else
