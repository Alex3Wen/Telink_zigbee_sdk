...
#define MAC_STATE_FOR_LAYER( layer )                                    \
    (                                                    \
    (zb_uint8_t) ZCTRL->mac_ctx.state[ (layer) ] + 0                                \
    )
...
{                                                               \
  TRACE_MSG(TRACE_MAC2, "mac set state %hd %hd -> %hd", (FMT__H_H_H, layer, ZCTRL->mac_ctx.state[ (layer) ], newstate)); \
  ZCTRL->mac_ctx.state[ (layer) ] = (newstate);                \
  MAC_DUMP_STATES();                                            \
}
