...
#define ZB_NSDU_SIZE_LINK_STATUS_DEFAULT 2
#define ZB_NSDU_AUX_HDR_LEN       14
...
#define ZB_DECLARE_SIMPLE_DESC(in_clusters_count, out_clusters_count)   \
  ...
  {                                                                     \
    ...
    zb_uint16_t   app_input_cluster_list[in_clusters_count];  \
    zb_uint8_t    app_output_cluster_count; /* Application output cluster count */ \
    zb_uint16_t   app_output_cluster_list[out_clusters_count]; /* Application input and output cluster list */ \
  } ZB_PACKED_STRUCT                                                    \
...
typedef struct {
	...
	zb_uint8_t    reserved:4;               /* Reserved */              
	zb_uint8_t    app_input_cluster_count;  /* Application input cluster count */ 
	zb_uint16_t   *input_cluster_list;      /* Application input cluster list */
	zb_uint8_t    app_output_cluster_count; /* Application output cluster count */ 
	zb_uint16_t   *output_cluster_list;     /* Application output cluster list */
} zb_af_simple_desc_t;

ZB_DECLARE_SIMPLE_DESC(1,1); /* General descriptor type */
ZB_DECLARE_SIMPLE_DESC(7,8); /* ZDO descriptor type */

#define ZB_MAX_EP_NUMBER 2 /* max supported EP number, increase if needed */
 ...
 */
typedef void (*zb_endpoint_cb_t)(zb_uint8_t *);

typedef struct {
	zb_uint8_t endpoint;
	zb_endpoint_cb_t cb;
	zb_af_simple_desc_t* pSimpleDesc;
...
/**
  Adds endpoint descriptor.
  @param endpoint - specified endpoint ID
  @param simple_desc - pointer to simple descriptor to add
  @param cb - pointer to the callback function to this endpoint
  @return RET_OK, RET_OVERFLOW
 */
zb_ret_t zb_add_endpoint_descriptor(zb_uint8_t endpoint, zb_af_simple_desc_t *simple_desc, zb_endpoint_cb_t cb);
