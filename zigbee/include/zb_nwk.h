...
// Link Status Option definitions
#define ZB_LINK_STATUS_OPTION_LAST_FRAME                0x40
#define ZB_LINK_STATUS_OPTION_FIRST_FRAME               0x20
#define ZB_LINK_STATUS_OPTION_ENTRY_COUNT_MASK          0x1F
#define ZB_LINK_STATUS_OPTION_RESERVED                  0x80
 ...
 */
#ifndef ZB_NS_BUILD
#define ZB_LINK_QUALITY_IS_OK_FOR_JOIN(lqi) 1//((lqi) >= 3) //(((lqi) / (256 / 8)) >= 3)
#else
 ...
 */
#define ZB_NWK_HDR_CUT(packet, ptr, type)                   \
do                                                    \
{                                                     \
  zb_nwk_hdr_t *_nwk_hdr = (zb_nwk_hdr_t *)ZB_BUF_BEGIN(packet); \
  ZB_BUF_CUT_LEFT(packet, ZB_NWK_HDR_SIZE(_nwk_hdr->frame_control), ptr, type);   \
} while (0)
...
#define ZB_NWK_ALLOC_COMMAND_GET_PAYLOAD_PTR(buf, cmd_id, cmd_size, payload, type) \
  {                                                                     \
    zb_uint8_t *_ptr;                                                   \
    ZB_BUF_ALLOC_RIGHT(buf, ZB_NWK_COMMAND_SIZE(cmd_size), _ptr, type);       \
    *_ptr = (cmd_id);                                                   \
