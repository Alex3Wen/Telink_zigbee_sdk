...
#ifndef ZB_CC25XX
#ifndef TC32_SPECIFIC_INCLUDED
#define TC32_SPECIFIC_INCLUDED
 ...
 */
#define ZB_TRANS_RECV_PACKET(buf) zb_tc32_fifo_read(0, buf, 0)
#define ZB_TRANS_READ_TXNFIFO(buf, a)            \
  zb_tc32_fifo_read(1, buf, a)
...
/**
   ...
   @param addr - register address
 */
#define ZB_READ_SHORT_REG(addr) zb_spidev_short_read(addr, &ZCTRL->mac_ctx.rw_reg.value.byte_value)
...
/**
   ...
   @param addr - register address
 */
#define ZB_READ_LONG_REG(addr) zb_spidev_long_read(addr, &ZCTRL->mac_ctx.rw_reg.value.byte_value)
...
#define ZB_UBEC_SET_RX_DATA_STATUS() (TRANS_CTX().int_status |= 0x08)  /* get bit 3 */
...
#define ZB_MAC_GET_BYTE_VALUE() ZCTRL->mac_ctx.rw_reg.value.byte_value
...
#define ZB_SET_PENDING_BIT() \
    {                        \
		u8 pending = 1;      \
		rf_set(RF_ID_MAC_FRAME_PENDING, (u8*)&pending, 1); \
	}
...
#define ZB_SET_MAC_RECV() \
    {                        \
		u8 recvFalg = 1;      \
		rf_set(RF_ID_MAC_RECV_START, (u8*)&recvFalg, 1); \
	}

#define ZB_CLEAR_MAC_RECV() \
    {                        \
		u8 recvFalg = 0;      \
		rf_set(RF_ID_MAC_RECV_START, (u8*)&recvFalg, 1); \
	}
...
void zb_tc32_standby();
void zb_tc32_register_wakeup();
...
void zb_tc32_fifo_write(zb_buf_t *buf) ;
...
/**
   Read from FIFO
 */
void zb_tc32_fifo_read(zb_uint8_t tx_fifo, zb_buf_t *buf, zb_uint8_t len);
 ...
 */
void zb_tc32_check_int_status();

/**
   Portable macro for int status check routine
 */
#define ZB_CHECK_INT_STATUS() zb_tc32_check_int_status()
...
void zb_tc32_rfRecvCb(void* arg);
...
/**
   Assign short pan ID in TC32

   @param pan_id - new pan id
 */

#define ZB_TRANSCEIVER_SET_PAN_ID(pan_id)  rf_set(RF_ID_MAC_PANID, (u8*)&pan_id, 2)
...
#define ZB_UPDATE_LONGMAC() \
  rf_set(RF_ID_MAC_EXT_ADDR, (u8*)&(MAC_PIB().mac_extended_address), 8) 


/**
   Update TC32 pan id: copu PIB value to the transiver
 */
...
/**
   Update TC32 short address: copu PIB value to the transiver
 */
#define ZB_UPDATE_SHORT_ADDR() rf_set(RF_ID_MAC_SHORT_ADDR, (u8*)&(MAC_PIB().mac_short_address), 2) 
...
{\
	...
	ZB_BUF_INITIAL_ALLOC( ZCTRL->mac_ctx.operation_buf, 1, data_ptr );\
	ZB_ASSERT(data_ptr);\
	data_ptr[0] = (ZB_SREG_ISRSTS << 1);\
	ZB_DUMP_OUTGOING_DATA(ZCTRL->mac_ctx.operation_buf);\
	data_ptr[0] = MAC_CTX().regs_queue.ring_buf[MAC_CTX().regs_queue.read_i]->isrsts;\
    ZB_DUMP_INCOMING_DATA(ZCTRL->mac_ctx.operation_buf);\
