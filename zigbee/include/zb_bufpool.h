...
#define ZB_IN_BUF_AVAILABLE() (ZCTRL->bpool.bufs_allocated[1] < ZB_IOBUF_POOL_SIZE/2)
#define ZB_OUT_BUF_AVAILABLE() (ZCTRL->bpool.bufs_allocated[0] < ZB_IOBUF_POOL_SIZE/2)
...
#define ZB_BUF_INITIAL_ALLOC(zbbuf, size, ptr, type) (ptr) = (type)zb_buf_initial_alloc((zbbuf), (size))
 ...
 */
#define ZB_BUF_ALLOC_LEFT(zbbuf, size, ptr, type) (ptr) = (type)zb_buf_smart_alloc_left((zbbuf), (size))
 ...
 */
#define ZB_BUF_ALLOC_RIGHT(zbbuf, size, ptr, type) (ptr) = (type)zb_buf_smart_alloc_right((zbbuf), (size))
...
/**
   ...
   @param ptr   - (out) pointer to the new buffer begin
 */
#define ZB_BUF_CUT_LEFT(zbbuf, size, ptr, type)  (ptr) = (type)zb_buf_cut_left((zbbuf), (size)) 
...
#define ZB_BUF_FROM_REF(ref) (&ZCTRL->bpool.pool[ref])

#define ZB_REF_FROM_BUF(buf) (buf - &ZCTRL->bpool.pool[0])
